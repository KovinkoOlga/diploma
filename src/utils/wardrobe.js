...
import { getWardrobeTemplateById, wardrobeTemplates } from "../data/wardrobeTemplates";
...
function buildDraftFromTemplate(template, catalogId, sourceType) {
  return normalizeWardrobeItemDraft({
    title: template.title,
    image: template.image ?? placeholderImage,
    catalogId: catalogId ?? "main",
    ...
    sourceType,
    recognitionLabel:
      sourceType === "catalog" ? "Шаблон из базового каталога" : "Мок-распознавание после обработки изображения",
  });
}

export function createMockProcessedDraft({ sourceType = "photo", catalogId = "main", templateId } = {}) {
  const fallbackTemplateId = sourceType === "gallery" ? "template_5" : "template_1";
  const template = getWardrobeTemplateById(templateId ?? fallbackTemplateId) ?? wardrobeTemplates[0];

  return buildDraftFromTemplate(template, catalogId, sourceType);
}

export function createDraftFromTemplate(templateId, catalogId = "main") {
  const template = getWardrobeTemplateById(templateId) ?? wardrobeTemplates[0];
  return buildDraftFromTemplate(template, catalogId, "catalog");
}
